...
describe('PDF visual regression test', () => {
  test('standard template', async () => {
    const html = fs.readFileSync(`./tests/examples/standard.html`, {encoding: 'utf8'});
    const actualPdfBuffer = await generate(html);
    await writeBuffer(actualPdfBuffer, `${config.paths.actualPdfRootFolder}/standard.pdf`);

    const comparePdf = new ComparePdf(config);
    const result = await comparePdf.actualPdfFile('standard.pdf').baselinePdfFile('standard.pdf').compare();
    expect(result?.status).toBe('passed');
  });

  test('elegant template', async () => {
    const html = fs.readFileSync(`./tests/examples/elegant.html`, {encoding: 'utf8'});
    const actualPdfBuffer = await generate(html);
    await writeBuffer(actualPdfBuffer, `${config.paths.actualPdfRootFolder}/elegant.pdf`);

    const comparePdf = new ComparePdf(config);
    const result = await comparePdf.actualPdfFile('elegant.pdf').baselinePdfFile('elegant.pdf').compare();
    expect(result?.status).toBe('passed');
  });
});
