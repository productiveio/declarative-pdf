export default function evalShowOnlyPageWithSection(
  ...
  if (docPage) {
    [
      ...
      const element = docPage.querySelector(elementName) as HTMLElement | undefined;
      if (!element) return;
      ...
      if (physicalPageIndex !== undefined && elementName === section) {
        const sectionElements = element.querySelectorAll('physical-page') as NodeListOf<HTMLElement>;
        ...
        Array.from(sectionElements).forEach((sectionEl, idx) => {
          sectionEl.style.display = idx === physicalPageIndex ? 'block' : 'none';
        });
      }
    });
  }
