export default function evalInjectCurrentPageNumber(
  ...
  const docPage = document.querySelector(`document-page:nth-child(${docPageIndex + 1})`) as HTMLElement;
  const section = docPage.querySelector(sectionName) as HTMLElement;

  Array.from(section.querySelectorAll('current-page-number') as NodeListOf<HTMLElement>).forEach((el) => {
